...
โดย Leg index:
\begin{itemize}
    \item FL = 0 → Motors 1, 2
    \item FR = 1 → Motors 3, 4
    \item RL = 2 → Motors 5, 6
    \item RR = 3 → Motors 7, 8
\end{itemize}
...
\begin{itemize}
    \item Power distribution: 24V → 8 motor controllers
    \item Serial communication: PC ← USB-Serial → MCU hub → 8 motor controllers
...
\begin{itemize}
    \item \textbf{Threading:} Python Threading เหมาะสำหรับ Multi-motor control
    \item \textbf{Mirror Kinematics:} การ Mirror X-coordinate ทำงานได้ดี
    \item \textbf{Motor Indexing:} ระบบตั้งชื่อที่ดีช่วยลดความสับสน
    \item \textbf{Communication Bandwidth:} 921600 baud เพียงพอสำหรับ 8 motors @ 100 Hz
    \item \textbf{Compromised Posture:} พารามิเตอร์เริ่มต้นต้องปรับจูน (→ Phase 5.2)
\end{itemize}
...
\begin{enumerate}
    \item \textbf{เพิ่ม Step Length:} จาก 30 mm → 50-60 mm
    \item \textbf{เพิ่ม Lift Height:} จาก 15 mm → 30 mm
    \item \textbf{Asymmetric Trajectory:} ใช้ Stance 65\% / Swing 35\%
    \item \textbf{Multi-Mode Gait:} พัฒนา TROT, SMOOTH\_TROT, BACKWARD\_TROT, WALK, CRAWL
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Foot Orientation:} ปลายเท้าชี้ไปด้านหน้า → อาจส่งผลต่อแรงงัดเมื่อสัมผัสพื้น
    \item \textbf{Ground Contact:} ต้องพิจารณา Heel-strike vs Toe-first landing
    \item \textbf{Power Budget:} 8 motors @ 1-3 A each → ต้องมี Battery หรือ Power supply ที่เพียงพอ
    \item \textbf{Thermal Management:} มอเตอร์ต้องไม่เกิน 80°C
\end{itemize}
