...
\begin{itemize}
    \item \textbf{Step length ต่ำ (30 mm):} ขาไม่เหยียดออก → ท่าทางหดตัว
    \item \textbf{Lift height ต่ำ (15 mm):} ไม่ยกเท้าสูงพอ → เท้าอาจกระทบพื้น
    \item \textbf{Center position:} อาจไม่เหมาะสมกับการเดินจริง
\end{itemize}

\subsubsection{Lack of Smoothness}
\begin{itemize}
    \item \textbf{Symmetric trajectory:} Stance และ Swing ใช้เวลาเท่ากัน (50\%-50\%)
    \item \textbf{แรงกระแทก:} เท้าสัมผัสพื้นแบบ sudden impact
    \item \textbf{Jerk สูง:} ความเร่งเปลี่ยนแปลงเร็ว
\end{itemize}
...
\toprule
\textbf{Mode} & \textbf{Dir} & \textbf{Step} & \textbf{Lift} & \textbf{Characteristics} \\
\midrule
TROT & ⬆️ & 50 mm & 15 mm & Fast, aggressive \\
SMOOTH\_TROT & ⬆️ & 50 mm & 30 mm & Balanced \& gentle \\
BACKWARD\_TROT & ⬇️ & 50 mm & 30 mm & Smooth reverse \\
WALK & ⬆️ & 40 mm & 25 mm & Slow, stable \\
CRAWL & ⬆️ & 30 mm & 20 mm & Very stable \\
STAND & - & - & - & Static testing pose \\
\bottomrule
\end{tabular}
\caption{โหมดการเดินทั้ง 6 โหมด}
\end{table}
...
\subsubsection{ลักษณะ}
\begin{itemize}
    \item[\checkmark] ความเร็วสูง (~100 mm/s)
    \item[\times] แรงกระแทกสูง
    \item[\times] ท่าทางค่อนข้าง aggressive
    \item[\checkmark] เหมาะสำหรับพื้นเรียบ
\end{itemize}
...
\subsubsection{ลักษณะ}
\begin{itemize}
    \item[\checkmark] ความเร็วปานกลาง (~80 mm/s)
    \item[\checkmark] แรงกระแทกต่ำ (ลดลง ~40\%)
    \item[\checkmark] ท่าทางนุ่มนวล
    \item[\checkmark] สมดุลระหว่างความเร็วและความนุ่มนวล
    \item[\checkmark] \textbf{โหมดแนะนำสำหรับการใช้งานทั่วไป}
\end{itemize}
...
\subsubsection{ลักษณะ}
\begin{itemize}
    \item[\checkmark] ความเร็ว ~80 mm/s (ถอยหลัง)
    \item[\checkmark] \textbf{นุ่มนวลที่สุด!} (ตามข้อสังเกตจากการทดสอบ)
    \item[\checkmark] Impact force ต่ำมาก
    \item[\checkmark] เหมาะสำหรับทดสอบและปรับจูน
\end{itemize}

\textbf{ข้อสังเกตพิเศษ:} การเดินถอยหลังให้ผลดีกว่าเดินหน้า → น่าจะเกี่ยวข้องกับ Foot orientation (ดูรายละเอียด Section 5)
...
\begin{itemize}
    ...
    \item \textbf{Gait Pattern:} Sequential (FL → FR → RL → RR)
    \item \textbf{Phase Offset:} 0°, 90°, 180°, 270°
\end{itemize}

\subsubsection{ลักษณะ}
\begin{itemize}
    \item[\checkmark] ความเร็วช้า (~50 mm/s)
    \item[\checkmark] เสถียรมาก (3 ขายึดพื้นเสมอ)
    \item[\checkmark] แรงกระแทกต่ำ
    \item[\times] ช้า
\end{itemize}
...
\subsubsection{ลักษณะ}
\begin{itemize}
    \item[\checkmark] ความเร็วช้ามาก (~25 mm/s)
    \item[\checkmark] เสถียรที่สุด
    \item[\checkmark] เหมาะสำหรับพื้นไม่เรียบ
    \item[\times] ช้ามาก
\end{itemize}
...
\begin{itemize}
    ...
    \item ปลายเท้าชี้ไปข้างหน้า → มุมปะทะกับพื้นมาก
...
\begin{itemize}
    ...
    \item Forward walking: $\theta_{contact}$ ใหญ่ → $F_{impact}$ สูง
    \item Backward walking: $\theta_{contact}$ เล็ก → $F_{impact}$ ต่ำ
\end{itemize}
...
\toprule
\textbf{Mode} & \textbf{Speed} & \textbf{Impact} & \textbf{Smooth} & \textbf{Stable} & \textbf{Rating} \\
 & (mm/s) & Force & ness & ness & (1-5) \\
\midrule
TROT & 100 & High & ⭐⭐ & ⭐⭐⭐ & 3/5 \\
SMOOTH\_TROT & 80 & Med & ⭐⭐⭐⭐ & ⭐⭐⭐⭐ & \textbf{4.5/5} \\
BACKWARD\_TROT & 80 & Low & ⭐⭐⭐⭐⭐ & ⭐⭐⭐⭐ & \textbf{5/5} \\
WALK & 50 & Low & ⭐⭐⭐⭐ & ⭐⭐⭐⭐⭐ & 4/5 \\
CRAWL & 25 & V.Low & ⭐⭐⭐ & ⭐⭐⭐⭐⭐ & 3.5/5 \\
\bottomrule
\end{tabular}
\caption{เปรียบเทียบประสิทธิภาพของแต่ละโหมด}
\end{table}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Stance phase นาน → ลดแรงกระแทก
        \item Swing phase สั้น → เพิ่มความเร็ว
...
\begin{itemize}
    \item เพิ่ม DOF ที่ข้อเท้า (3-DOF per leg → 3+1 DOF)
    \item ควบคุมมุมปลายเท้าแบบ Active
    \item ปรับมุมตาม Ground contact phase
