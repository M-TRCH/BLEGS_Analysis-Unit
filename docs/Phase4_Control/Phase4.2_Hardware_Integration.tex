...
\subsubsection{Command Packet (PC → MCU)}
...
\subsubsection{Response Packet (MCU → PC)}
...
ถ้าไม่ได้รับ Response ภายใน Timeout period:

\begin{itemize}
    \item \textbf{Timeout period:} 100 ms
    \item \textbf{Action:} Retry 3 ครั้ง
    \item \textbf{Fallback:} ถ้า retry ครบแล้วยังไม่สำเร็จ → Emergency stop
\end{itemize}

\subsubsection{Packet Loss Recovery}

\begin{enumerate}
    \item ตรวจสอบ CRC ของ Packet ที่รับได้
    \item ถ้า CRC ไม่ตรง → ทิ้ง Packet และขอส่งใหม่
    \item บันทึก Error rate สำหรับการวิเคราะห์
\end{enumerate}
...
\begin{itemize}
    \item ทดสอบ 50 Hz → Jitter ลดลง 50\%
    \item ทดสอบ 100 Hz → Jitter ลดลง 30\%
    \item ทดสอบ 200 Hz → Jitter รุนแรงมาก
\end{itemize}

\textbf{สรุป:} Update rate สูงเกินไป → มอเตอร์ไม่ทันตอบสนอง → เกิด Oscillation

\textbf{ทดสอบที่ 2: วิเคราะห์ Trajectory}
\begin{itemize}
    \item Trajectory มี Discontinuity (กระโดด)
    \item ความเร่งเปลี่ยนแปลงแบบ step change
    \item Jerk สูง (ความเร่งเปลี่ยนทันที)
\end{itemize}

\textbf{สรุป:} Trajectory ไม่นุ่มนวลพอ → มอเตอร์พยายามติดตาม → เกิด Jitter

\subsubsection{วิธีแก้ไข}

\textbf{Solution 1: ปรับ Update Rate}
\begin{itemize}
    \item ลดจาก 200 Hz → 100 Hz
    \item ผล: Jitter ลดลง ~30\%
\end{itemize}
...
หลังจากใช้ทั้ง 3 solutions ร่วมกัน:
\begin{itemize}
    \item[\checkmark] Jitter ลดลง ~90\%
    \item[\checkmark] เสียงเบาลงมาก
    \item[\checkmark] Current draw ลดลง ~25\%
    \item[\checkmark] อุณหภูมิมอเตอร์ลดลง 8-10°C
    \item[\checkmark] Success rate เพิ่มขึ้นจาก 85\% → 96-99\%
\end{itemize}
...
\begin{itemize}
    \item เพิ่ม RX buffer จาก 64 bytes → 256 bytes
    \item เพิ่ม TX buffer จาก 64 bytes → 256 bytes
\end{itemize}
...
\begin{itemize}
    \item[\checkmark] Timeout rate ลดลงจาก 3-5\% → $< 1$\%
    \item[\checkmark] Communication เสถียรขึ้น
    \item[\checkmark] ไม่พบ Buffer overflow
\end{itemize}
